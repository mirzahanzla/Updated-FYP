...
const TaskTableContactUs = ({ DATA, pagination, setPagination, pageCount, totalUsers ,Update ,setUpdate}) => {
  ...
  return (
    <>
      ...
      {selectedRowData && <UserContactUs  rowData={selectedRowData} onClose={() => setselectedRowData(null)} />}
  ...
  return (
    <div className='px-2 rounded-xl'>
      {table.getRowModel().rows.map((row) => (
        <div className="tr flex justify-around cursor-pointer my-5" key={row.id}>
          <div className="td">
            <input
              type="checkbox"
  ...
  return <>
    ...
    <div className='px-2 bg-[#F7F8FA] mx-auto rounded-xl'>
      {table.getHeaderGroups().map((headerGroup) => (
        <div className="tr  flex justify-around" key={headerGroup.id}>
          <div className="th">
            <input
...
export default TaskTableContactUs;
