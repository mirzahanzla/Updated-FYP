...
const prepareChartData = (blogs, numMonths = 12) => {
  ...
  months.forEach((month) => {
    monthlyData[month] = { likes: 0, comments: 0, visits: 0 };
  });

  // Store data for each month to show in line graph
  blogs.forEach((blog) => {
    if (blog.monthlyInteraction) {
      Object.entries(blog.monthlyInteraction).forEach(([month, interactions]) => {
        if (monthlyData[month]) {
          monthlyData[month].likes += interactions.likes || 0;
          monthlyData[month].visits += Array.isArray(interactions.visits) ? interactions.visits.length : 0; // Check for array
          
          if (interactions.commentedBy) {
            Object.values(interactions.commentedBy).forEach((comments) => {
              monthlyData[month].comments += Array.isArray(comments) ? comments.length : 0; // Check for array
            });
          }
        }
      });
  ...
  // Sort and convert to readable month form
...
const EngagementGrowth = ({ blogs = [] }) => {
  // Fallback for empty blogs to prevent runtime errors
  const chartData = blogs.length > 0 ? prepareChartData(blogs) : [];

  return (
    <div className="bg-white w-full mt-10 rounded-3xl" id='Audience'>
